...
**Key invariant**: `crates/core/` must NEVER import from `crates/ts/`,
`crates/java/`, or `crates/llm/`. Core defines traits; language crates
implement them.
...
4. **Pipeline extension data goes in `Language::AnalysisExtensions`.** TypeScript
   has `TsAnalysisExtensions` (SD pipeline results). Java has
   `JavaAnalysisExtensions` (SD pipeline results). The orchestrator interacts
   with extensions only through `run_extended_analysis`,
...
Every edge has a `strength: EdgeStrength` field with **four variants**
(`Allowed`, `Structural`, `Wrapper`, `Required`) encoding two independent
...
**Four `EdgeStrength` variants:**

| Variant | CHP | PMC | Generated Rules |
|---------|-----|-----|-----------------|
| `Required` | YES | YES | Both `notParent` and `requiresChild` |
| `Structural` | YES | NO | Only `notParent` |
| `Wrapper` | NO | YES | Only `requiresChild` |
...
#### Composition Tree & Edge Verification Data

Single-component families (50), composition tree ground truth tables
(55 families), edge ground truth (78 edges across 4 categories A-D),
and known tree/conformance rule issues are maintained in
[`composition-ground-truth.md`](composition-ground-truth.md).

Consult that file when validating composition tree correctness or
debugging conformance rule false positives.
...
```sh
cargo test -p semver-analyzer-ts --lib    # unit tests
cargo test -p semver-analyzer-ts          # + integration tests
cargo test --workspace                    # full suite (~1,200+ tests)
```
...
Implemented in `crates/core/src/git.rs`, re-exported by
`crates/ts/src/git_utils.rs`. Return `Option<String>`. Log at `trace!`
level on failure. **Do not duplicate these functions** — there were
previously 4+ copies across the codebase.
